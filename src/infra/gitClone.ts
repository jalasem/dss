 ...
 *
 * The `--` end-of-options separator (before `url`) is a defense-in-depth
 * layer against argument injection: `parseGitUrl` already rejects any URL
 * starting with `-` (the primary defense — see core/gitUrl.ts), but `--`
 * guarantees git treats BOTH `url` and `dest` as strictly positional
 * arguments — never parsed as flags — even if some future caller ever
 * reaches this function with an unvalidated string.
 */
  ...
  return new Promise((resolve, reject) => {
    ...
    const child = execFile('git', ['clone', '--', url, dest], execOptions, (error, _stdout, stderr) => {
      if (error) {
        ...
        reject(new GitCloneError(`git clone failed: ${stderrTail}`, stderrTail));
        return;
      }
      resolve();
    });
